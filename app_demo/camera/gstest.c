...
//create element, caps and bus
  GMainLoop *loop;
  GstElement *pipeline, *source, *converter, *encoder, *decoder, *demux, *mux, *sink, *vqueue, *parse;
...
if ((argc < 3)||(argc > 4))
{
    g_printerr("Usage:%s [1] [camera device name] [video storage location] for camera video record or\n", argv[0]);
    g_printerr("Usage:%s [2] [camera device name] for camera capture or\n", argv[0]);
  ...
  if(option == 1)
  {
    ...
    source = gst_element_factory_make("v4l2src", "camera-input");
       g_object_set (source, "num-buffers", 1800, NULL);
